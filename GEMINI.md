...
CRITICAL PITFALL: trading_shared Package Installation

When adding new modules to trading_shared (e.g., logging_utils.py), you MUST:

1. Place the file in src/trading_shared/ directory (not the root trading_shared/ directory)
2. Reinstall the package: pip install -e c:\trading\trading_shared
3. Verify import works: python -c "from trading_shared.module_name import ..."

This pitfall occurs frequently because:
- Files placed in trading_shared/ root are not part of the installed package
- The package structure is src/trading_shared/ (defined in pyproject.toml)
- Import errors will occur if files are in the wrong location

Always verify new trading_shared modules are importable before updating dependent services.
